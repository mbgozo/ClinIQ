...
export default function LeaderboardPage() {
      ...
      <div className="flex h-[80vh] items-center justify-center">
        <div className="flex flex-col items-center gap-10">
          <div className="relative h-32 w-32 flex items-center justify-center">
             ...
             />
             <Icons.Orbit className="h-10 w-10 text-emerald-500 animate-pulse" />
          </div>
      ...
      <div className="flex h-[80vh] items-center justify-center">
        ...
        >
          <div className="h-24 w-24 bg-white/5 text-amber-500 rounded-[2rem] flex items-center justify-center mx-auto mb-8 border border-white/10 shadow-3xl">
            <Icons.Info className="h-12 w-12" />
          </div>
  ...
  const getRankData = (rank: number) => {
    switch (rank) {
      case 1: return { 
        icon: <Icons.Crown className="h-10 w-10" />, 
      ...
      case 3: return { 
        icon: <Icons.Medal className="h-7 w-7" />, 
              ...
              <div className={cn(
                ...
                {isWinner && (
                  <>
                    ...
                    <Icons.Sparkles className="absolute top-10 right-10 h-10 w-10 text-amber-400/20 animate-pulse" />
                  </>
                )}
                ...
                <div className={cn(
                  ...
                  <div className={cn(
                    ...
                    <div className="h-full w-full bg-slate-50 flex items-center justify-center relative overflow-hidden">
                       <Icons.Fingerprint className={cn("text-slate-100", isWinner ? "h-32 w-32" : "h-24 w-24")} />
                       {isWinner && (
                ...
                <div className="text-center space-y-4 mb-14">
                   <div className="flex items-center justify-center gap-3">
                      <Icons.Star className={cn("h-4 w-4 fill-current", isWinner ? "text-amber-400" : "text-slate-200")} />
                   ...
                   <div className="flex flex-col items-center gap-1.5 pt-2">
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                       <Icons.Hexagon className="h-3 w-3 text-emerald-500" /> {user.institution}
                     </p>
              ...
              >
                ...
                <div className={cn(
                  ...
                  isOwn ? "bg-white/10 border-white/10" : "bg-slate-50 border-slate-100"
                   ...
                   <Icons.Fingerprint className={cn("h-10 w-10", isOwn ? "text-white/20" : "text-slate-200 group-hover:text-emerald-400")} />
                   {isOwn && (
                ...
                <div className="hidden lg:flex px-12 items-center gap-4 min-w-[200px]">
                   <Icons.ShieldCheck className={cn("h-6 w-6", isOwn ? "text-emerald-400" : "text-emerald-600")} />
                ...
                <div className={cn(
                  ...
                  <Icons.ArrowUpRight className="h-7 w-7 transform group-hover:scale-110 transition-transform" />
                </div>
              </motion.div>
            );
          })}
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center gap-24">
          <div className="relative group/prestige">
             <div className="h-44 w-44 bg-slate-900 text-white rounded-[4rem] flex items-center justify-center shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] rotate-6 group-hover/prestige:rotate-0 transition-transform duration-1000 border-4 border-white/5">
                <Icons.Crown className="h-24 w-24 text-emerald-400" />
             </div>
             <div className="absolute -bottom-8 -right-8 h-24 w-24 bg-white rounded-[2rem] shadow-3xl flex items-center justify-center border-3 border-slate-50">
                <Icons.Star className="h-10 w-10 text-amber-500 animate-spin-slow" />
             </div>
          </div>
          <div className="flex-1 space-y-12 text-center lg:text-left">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-12 pt-4">
              {[
                { label: "Answer Matrix", pts: "+10 XP", icon: <Icons.MessageSquare className="h-6 w-6" />, color: "text-emerald-500" },
                { label: "Heuristic Validated", pts: "+50 XP", icon: <Icons.Zap className="h-6 w-6" />, color: "text-amber-500" },
                { label: "Scholarship Signal", pts: "+5 XP", icon: <Icons.TrendingUp className="h-6 w-6" />, color: "text-indigo-600" },
                { label: "Operational Hub", pts: "+20 XP", icon: <Icons.Network className="h-6 w-6" />, color: "text-emerald-900" }
              ].map((item, i) => (
